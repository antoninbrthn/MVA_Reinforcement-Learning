...
class LQG1D(object):
    ...
    def __init__(self, initial_state_type):
        ...
        # hashing of the states and actions
        self.n_bucket_state = 10
        self.n_bucket_action = 10
        self.state_action_visits = np.ones((self.n_bucket_state, self.n_bucket_action))

        # hyperparameter for bonus function
        self.beta = 1

    def update_visits(self, s, a):
        self.state_action_visits[s, a] +=  1

    def hash(self, s, a):
        state_buckets = np.linspace(-self.max_pos, self.max_pos, self.n_bucket_state)
        action_buckets = np.linspace(-self.max_action, self.max_action, self.n_bucket_action)

        closest_state = np.argmin([abs(s - bucket) for bucket in state_buckets])
        closest_action = np.argmin([abs(a - bucket) for bucket in action_buckets])

        return closest_state, closest_action

    def step(self, action, render=False, use_bonus = True):
        u = np.clip(action, -self.max_action, self.max_action)

        # Update visit counts
        s, a = self.hash(self.state, u)
        bonus = self.bonus(s, a)
        self.update_visits(s, a)
    ...
    def bonus(self, state, action):
        return self.beta * (1/self.state_action_visits[state, action])**0.5
